...
public class WebSecurityConfig {
        ...
        public SecurityFilterChain leaderFilterChain(HttpSecurity http) throws Exception {
                ...
                http.authorizeHttpRequests(auth -> auth
                                .requestMatchers("/login/official", "/register", "/assets/**", "/403", "/404")
                                .permitAll()
                                .requestMatchers("/", "/login/official", "/register", "/assets/**", "/403", "/404")
                                .permitAll()
        ...
        public SecurityFilterChain officialCommonFilterChain(HttpSecurity http) throws Exception {
                ...
                http.authorizeHttpRequests(auth -> auth
                                .requestMatchers("/login/officer", "/process-login-official", "/css/**", "/js/**")
                                .permitAll()
        ...
        public SecurityFilterChain citizenFilterChain(HttpSecurity http) throws Exception {
                http.securityMatcher("/", "/citizen/**", "/login/citizen", "/process-login", "/logout/citizen","/citizen/portal-submit-wizard");

                http.csrf(AbstractHttpConfigurer::disable);

                http.authorizeHttpRequests(auth -> auth
                                .requestMatchers("/", "/login/citizen", "/process-login", "/register", "/assets/**","/citizen/portal-submit-wizard",
                                                "/403", "/404")
                                .permitAll()
        ...
        public SecurityFilterChain specialistCommonFilterChain(HttpSecurity http) throws Exception {
                ...
                http.authorizeHttpRequests(auth -> auth
                                .requestMatchers("/login/specialist", "/process-login-official", "/css/**", "/js/**")
                                .permitAll()
                                .requestMatchers("/specialist/**")
                                .hasAnyRole("CANBO_TU_PHAP", "CANBO_DIA_CHINH", "CANBO_KINH_TE")
                                .anyRequest().authenticated());
                http.headers(headers -> headers.frameOptions(frameOptions -> frameOptions.sameOrigin()));
